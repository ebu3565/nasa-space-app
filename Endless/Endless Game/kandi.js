...
var collectibles = [];
var paperCollected = 0, plasticCollected = 0, ironCollected = 0;
...
function Collectible(x, y, type) {
  ...
  this.draw = function() {
    if (!this.collected) {
      ctx.drawImage(assetLoader.imgs[this.type], this.x, this.y, this.width, this.height);
    }
  };
}
Collectible.prototype = Object.create(Vector.prototype);
...
function updateCollectibles() {
  for (var i = 0; i < collectibles.length; i++) {
    ...
    if (!collectibles[i].collected && player.minDist(collectibles[i]) <= player.width/2 + 10) {
      collectibles[i].collected = true;
      
      // Update counters based on type
      switch(collectibles[i].type) {
        case 'paper': paperCollected++; break;
        case 'plastic': plasticCollected++; break;
        case 'iron': ironCollected++; break;
      }
      
      // Update UI
      document.getElementById('paper-count').textContent = paperCollected;
      document.getElementById('plastic-count').textContent = plasticCollected;
      document.getElementById('iron-count').textContent = ironCollected;
      
      // Play collection sound
      if (playSound) {
        assetLoader.sounds.collect.currentTime = 0;
        assetLoader.sounds.collect.play();
      }
    }
  ...
  collectibles = collectibles.filter(function(item) {
    return !item.collected && item.x > -platformWidth;
  });
}
...
function spawnSprites() {
  ...
  if (platformLength > 3 && rand(0, 20) === 0) {
    var collectibleType = ['paper', 'plastic', 'iron'][rand(0, 2)];
    collectibles.push(new Collectible(
      canvas.width + platformWidth % player.speed,
      platformBase - platformHeight * platformSpacer - 50,
      collectibleType
    ));
  }
...
function animate() {
  if (!stop) {
    ...
    background.draw();
        ...
        // Add this line with the other update functions
    updateCollectibles();
...
function startGame() {
  ...
  // Reset UI counters
  document.getElementById('paper-count').textContent = '0';
  document.getElementById('plastic-count').textContent = '0';
  document.getElementById('iron-count').textContent = '0';
...
function gameOver() {
  ...
  // Clear any previous collectibles data
  $('.collectibles-data').remove();
  
  // Add collection stats to game over screen
  var collectiblesHTML = '<p class="collectibles-data">Paper: ' + paperCollected + 
                       ' | Plastic: ' + plasticCollected + 
                       ' | Iron: ' + ironCollected + '</p>';
  
  // Insert after the h2 but before the button
  $('#score').after(collectiblesHTML);
...
function updateCollectibles() {
  for (var i = 0; i < collectibles.length; i++) {
    ...
    // Check for collection
    if (!collectibles[i].collected && player.minDist(collectibles[i]) <= player.width/2 + 20) {
      collectibles[i].collected = true;
      ...
      if (playSound) {
        // Add a collection sound here if desired
      }
    }
...
// Add to the game loop or updateCollectibles function
document.getElementById('paper-count').textContent = paperCollected;
document.getElementById('plastic-count').textContent = plasticCollected;
document.getElementById('iron-count').textContent = ironCollected;